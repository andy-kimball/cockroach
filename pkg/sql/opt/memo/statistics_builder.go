...
func (sb *statisticsBuilder) colStatFromChild(
	colSet opt.ColSet, nd RelNode, childIdx int,
	...
	child := nd.Child(childIdx).(RelNode)
...
func (sb *statisticsBuilder) colStatFromInput(
	...
	if lookupPrivate != nil || opt.IsJoinOp(nd) {
		intersectsLeft := nd.Child(0).(RelNode).Relational().OutputCols.Intersects(colSet)
		var intersectsRight bool
		if lookupPrivate != nil {
			intersectsRight = lookupPrivate.LookupCols.Intersects(colSet)
		} else {
			intersectsRight = nd.Child(1).(RelNode).Relational().OutputCols.Intersects(colSet)
		}
		if intersectsLeft {
			if intersectsRight {
			...
			return sb.colStatFromChild(colSet, nd, 0 /* childIdx */)
		}
		if intersectsRight {
			if lookupPrivate != nil {
				return sb.colStatTable(lookupPrivate.Table, colSet)
			}
			return sb.colStatFromChild(colSet, nd, 1 /* childIdx */)
		}
	...
	panic(fmt.Sprintf("unsupported node type %s", nd.Op()))
}
...
func (sb *statisticsBuilder) colStat(colSet opt.ColSet, nd RelNode) *props.ColumnStatistic {
	if colSet.Empty() {
	...
	if stat, ok := nd.Relational().Stats.ColStats.Lookup(colSet); ok {
		return stat
	}
	...
	switch nd.Op() {
	case opt.ScanOp:
		return sb.colStatScan(colSet, nd.(*ScanNode))

	case opt.VirtualScanOp:
		return sb.colStatVirtualScan(colSet, nd.(*VirtualScanNode))

	case opt.SelectOp:
		return sb.colStatSelect(colSet, nd.(*SelectNode))

	case opt.ProjectOp:
		return sb.colStatProject(colSet, nd.(*ProjectNode))

	case opt.ValuesOp:
		return sb.colStatValues(colSet, nd.(*ValuesNode))

	case opt.InnerJoinOp, opt.LeftJoinOp, opt.RightJoinOp, opt.FullJoinOp,
		opt.SemiJoinOp, opt.AntiJoinOp, opt.InnerJoinApplyOp, opt.LeftJoinApplyOp,
		opt.RightJoinApplyOp, opt.FullJoinApplyOp, opt.SemiJoinApplyOp, opt.AntiJoinApplyOp,
		opt.LookupJoinOp:
		return sb.colStatJoin(colSet, nd)

	case opt.UnionOp, opt.IntersectOp, opt.ExceptOp,
		opt.UnionAllOp, opt.IntersectAllOp, opt.ExceptAllOp:
		return sb.colStatSetNode(colSet, nd)

	case opt.GroupByOp, opt.ScalarGroupByOp, opt.DistinctOnOp:
		return sb.colStatGroupBy(colSet, nd)

	case opt.LimitOp:
		return sb.colStatLimit(colSet, nd.(*LimitNode))

	case opt.OffsetOp:
		return sb.colStatOffset(colSet, nd.(*OffsetNode))

	case opt.Max1RowOp:
		return sb.colStatMax1Row(colSet, nd.(*Max1RowNode))

	case opt.RowNumberOp:
		return sb.colStatRowNumber(colSet, nd.(*RowNumberNode))

	case opt.ZipOp:
		return sb.colStatZip(colSet, nd.(*ZipNode))

	case opt.ExplainOp, opt.ShowTraceForSessionOp:
		relProps := nd.Relational()
		return sb.colStatLeaf(colSet, &relProps.Stats, &relProps.FuncDeps)
	}

	panic(fmt.Sprintf("unrecognized relational expression type: %v", nd.Op()))
}
...
func (sb *statisticsBuilder) buildScan(scan *ScanNode, relProps *props.Relational) {
	s := &relProps.Stats
...
func (sb *statisticsBuilder) colStatScan(colSet opt.ColSet, scan *ScanNode) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildVirtualScan(scan *VirtualScanNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatVirtualScan(
	colSet opt.ColSet, scan *VirtualScanNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildSelect(sel *SelectNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatSelect(
	colSet opt.ColSet, sel *SelectNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildProject(prj *ProjectNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatProject(
	colSet opt.ColSet, prj *ProjectNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildJoin(
	join RelNode, relProps *props.Relational, h *joinPropsHelper,
) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
...
func (sb *statisticsBuilder) colStatJoin(colSet opt.ColSet, join RelNode) *props.ColumnStatistic {
	relProps := join.Relational()
	s := &relProps.Stats
	leftProps := join.Child(0).(RelNode).Relational()
	...
	if joinType == opt.LookupJoinOp {
		...
		rightProps = join.Child(1).(RelNode).Relational()
	}
...
func (sb *statisticsBuilder) buildIndexJoin(indexJoin *IndexJoinNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) buildGroupBy(groupNode RelNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatGroupBy(
	colSet opt.ColSet, groupNode RelNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildSetNode(setNode RelNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
	...
	leftStats := &setNode.Child(0).(RelNode).Relational().Stats
	rightStats := &setNode.Child(1).(RelNode).Relational().Stats
...
func (sb *statisticsBuilder) buildValues(values *ValuesNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatValues(
	colSet opt.ColSet, values *ValuesNode,
) *props.ColumnStatistic {
	...
	distinct := make(map[int64]struct{}, values.Rows[0].ChildCount())
...
func (sb *statisticsBuilder) buildLimit(limit *LimitNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatLimit(
	colSet opt.ColSet, limit *LimitNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildOffset(offset *OffsetNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatOffset(
	colSet opt.ColSet, offset *OffsetNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildMax1Row(max1Row *Max1RowNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatMax1Row(
	colSet opt.ColSet, max1Row *Max1RowNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildRowNumber(rowNum *RowNumberNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatRowNumber(
	colSet opt.ColSet, rowNum *RowNumberNode,
) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) buildZip(zip *ZipNode, relProps *props.Relational) {
	s := &relProps.Stats
	if zeroCardinality := s.Init(relProps); zeroCardinality {
		// Short cut if cardinality is 0.
		return
...
func (sb *statisticsBuilder) colStatZip(colSet opt.ColSet, zip *ZipNode) *props.ColumnStatistic {
...
func (sb *statisticsBuilder) copyColStatFromChild(
	colSet opt.ColSet, nd RelNode, s *props.Statistics,
) *props.ColumnStatistic {
	childColStat := sb.colStatFromChild(colSet, nd, 0 /* childIdx */)
...
func (sb *statisticsBuilder) ensureColStat(
	colSet opt.ColSet, maxDistinctCount float64, nd RelNode, relProps *props.Relational,
) *props.ColumnStatistic {
	s := &relProps.Stats

	colStat, ok := s.ColStats.Lookup(colSet)
	if !ok {
		colStat = sb.copyColStat(colSet, s, sb.colStatFromInput(colSet, nd))
	}
...
func (sb *statisticsBuilder) applyFilter(
	filters FiltersExpr, nd RelNode, relProps *props.Relational,
	...
	applyConjunct := func(conjunct *FiltersItem) {
		...
		scalarProps := conjunct.ScalarProps(nd.Memo())
		constrainedCols.UnionWith(scalarProps.OuterCols)
		if scalarProps.Constraints != nil {
			n := sb.applyConstraintSet(scalarProps.Constraints, nd, relProps)
...
func (sb *statisticsBuilder) applyConstraint(
	c *constraint.Constraint, nd RelNode, relProps *props.Relational,
) (applied bool) {
	...
	return sb.updateDistinctCountsFromConstraint(c, nd, relProps)
}

func (sb *statisticsBuilder) applyConstraintSet(
	cs *constraint.Set, nd RelNode, relProps *props.Relational,
	...
	for i := 0; i < cs.Length(); i++ {
		applied := sb.updateDistinctCountsFromConstraint(cs.Constraint(i), nd, relProps)
		if !applied {
...
func (sb *statisticsBuilder) updateDistinctCountsFromConstraint(
	...
	for col := 0; col <= prefix; col++ {
		...
		sb.ensureColStat(util.MakeFastIntSet(int(colID)), distinctCount, nd, relProps)
		applied = true
	}
...
func (sb *statisticsBuilder) applyEquivalencies(
	equivReps opt.ColSet, filterFD *props.FuncDepSet, nd RelNode, relProps *props.Relational,
) {
	equivReps.ForEach(func(i int) {
		equivGroup := filterFD.ComputeEquivGroup(opt.ColumnID(i))
		sb.updateDistinctCountsFromEquivalency(equivGroup, nd, relProps)
	})
}

func (sb *statisticsBuilder) updateDistinctCountsFromEquivalency(
	equivGroup opt.ColSet, nd RelNode, relProps *props.Relational,
) {
	s := &relProps.Stats
...
func (sb *statisticsBuilder) selectivityFromDistinctCounts(
	cols opt.ColSet, nd RelNode, s *props.Statistics,
) (selectivity float64) {
	selectivity = 1.0
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		...
		inputStat := sb.colStatFromInput(colStat.Cols, nd)
...
func (sb *statisticsBuilder) selectivityFromEquivalencies(
	equivReps opt.ColSet, filterFD *props.FuncDepSet, nd RelNode, s *props.Statistics,
) (selectivity float64) {
	selectivity = 1.0
	equivReps.ForEach(func(i int) {
		equivGroup := filterFD.ComputeEquivGroup(opt.ColumnID(i))
		selectivity *= sb.selectivityFromEquivalency(equivGroup, nd, s)
	})
	return selectivity
}

func (sb *statisticsBuilder) selectivityFromEquivalency(
	equivGroup opt.ColSet, nd RelNode, s *props.Statistics,
) (selectivity float64) {
	...
	equivGroup.ForEach(func(i int) {
		...
		colStat, ok := s.ColStats.Lookup(colSet)
		if !ok {
			colStat = sb.colStatFromInput(colSet, nd)
		}
...
// node. This is used for testing.
func RequestColStat(evalCtx *tree.EvalContext, rel RelNode, cols opt.ColSet) {
	var sb statisticsBuilder
	sb.init(evalCtx, rel.Memo().Metadata())
	sb.colStat(cols, rel)
}
