...
// interner interns relational and scalar nodes, which means that multiple
// equivalent nodes are mapped to a single in-memory instance. If two nodes with
// the same values for all fields are interned, the intern method will return
// the first node in both cases. Interned nodes can therefore be checked for
// equivalence by simple pointer comparison. Equivalence is defined more
// strictly than SQL equivalence; two values are equivalent only if their
// binary encoded representations are identical. For example, positive and
// negative float64 values *are not* equivalent, whereas NaN float values *are*
// equivalent.
//
// To use interner, first call the Init method to initialize storage. Call the
// appropriate Intern method to retrieve the canonical instance of that node.
// Release references to other instances, including the node passed to Intern.
//
// The interner computes a hash function for each node type that enables quick
// determination of whether a node has already been added to the interner. A
// hashXXX and isXXXEqual method must be added for every unique type of every
// field in every node type. Optgen generates Intern methods which use these
// methods to compute the hash and check whether an equivalent node is already
// present in the cache. Because hashing is used, interned nodes must remain
// immutable after interning. That is, once set, they can never be modified
// again, else their hash value would become invalid.
//
...
//
// Each node type follows the same interning pattern:
//
//   1. Compute an int64 hash value for the node using FNV-1a.
//   2. Do a fast 64-bit Go map lookup to determine if a node with the same hash
//      is already in the cache.
//   3. If so, then whether the existing node is equivalent, since there may be
//      a hash value collision.
//   4. Nodes with colliding hash values are linked together in a list. Rather
//      than use an explicit linked list data structure, colliding entries are
//      rehashed using a randomly generated hash value that is stored in the
//      existing entry.
//
// This pattern enables very low overhead hashing of nodes - the allocation of
// a Go map with a fast 64-bit key, plus a couple of reusable scratch byte
// arrays.
...
// Init is called again. Nodes interned before the call to Clear will not be
// connected to nodes interned after.
func (in *interner) Clear() {
	in.cache = nil
}

// Count returns the number of nodes that have been interned.
...
// pattern as that used by the node intern methods, with one difference. This
// intern method does not force the incoming physical properties to escape to
// the heap. It does this by making a copy of the physical properties before
...
func (in *interner) hashRelNode(val RelNode) {
	in.hash ^= internHash(uint64(reflect.ValueOf(val).Pointer()))
	in.hash *= prime64
}
...
func (in *interner) isRelNodeEqual(l, r RelNode) bool {
	return l == r
}
