...
// opsGen generates the enumeration of all node types.
type opsGen struct {
...
func (g *opsGen) genOperatorsByTag() {
	for _, tag := range g.compiled.DefineTags {
		...
		fmt.Fprintf(g.w, "func Is%sOp(nd Node) bool {\n", tag)
		fmt.Fprintf(g.w, "  switch nd.Op() {\n")
...
// sortDefines returns a copy of the given node definitions, sorted by name.
