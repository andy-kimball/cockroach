...
// TypeList is a list of SQL types.
type TypeList interface {
	// TypeAt returns the type at the given index in the list.
	TypeAt(i int) types.T

	// Length returns the number of types in the list.
	Length() int

	// Types returns a realized copy of the list, which may trigger a heap
	// allocation.
	Types() []types.T
}

// TypeSignature defines the input argument types and return type for a
// function. An overloaded function has multiple type signatures.
type TypeSignature interface {
	TypeList

	// Match checks if all types in the TypeSignature match the corresponding
	// elements in types.
	Match(types []types.T) bool
	// MatchAt checks if the parameter type at index i of the TypeSignature
	// matches type typ. In all implementations, types.Null will match with
	// each parameter type, allowing NULL values to be used as arguments.
	MatchAt(typ types.T, i int) bool
	// MatchLen checks that the TypeSignature can support l parameters.
	MatchLen(l int) bool
	// String returns a human readable signature
	String() string
}

var _ TypeSignature = ArgTypes{}
var _ TypeSignature = HomogeneousType{}
var _ TypeSignature = VariadicType{}
...
// Length implements the TypeSignature interface.
func (a ArgTypes) Length() int {
	return len(a)
}

// Types implements the TypeSignature interface.
...
// HomogeneousType is a TypeSignature implementation that accepts any
// arguments, as long as all are the same type or NULL. The homogeneous
// constraint is enforced in typeCheckOverloadedExprs.
...
// VariadicType is a TypeSignature implementation which accepts a fixed number
// of arguments at the beginning and an arbitrary number of homogenous
...
type ReturnTyper func(argTypes TypeList) types.T
