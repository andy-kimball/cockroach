...
func (u *UpdateChecker) CheckForUpdates(ctx context.Context) bool {
	...
	nodeInfo := u.collectNodeInfo(ctx)

	updatesURL := u.buildUpdatesURL(&nodeInfo)
	if updatesURL == nil {
...
func (u *UpdateChecker) collectNodeInfo(ctx context.Context) diagnosticspb.NodeInfo {
	n := diagnosticspb.NodeInfo{
		NodeID:      u.NodeID(),
		Build:       build.GetInfo(),
		Uptime:      int64(timeutil.Now().Sub(u.StartTime).Seconds()),
		LicenseType: getLicenseType(ctx, u.Settings),
	}
...
func (u *UpdateChecker) buildUpdatesURL(nodeInfo *diagnosticspb.NodeInfo) *url.URL {
	...
	return addInfoToURL(url, &clusterInfo, nodeInfo, &diagnosticspb.SQLInstanceInfo{})
}
